...
//Variables
//->WiFi Shield Vars
char ssid[] = "David HTC";  //Network SSID (name)
char pass[] = "velasquez12";  //Network password
int status = WL_IDLE_STATUS;  //Network status
//->TWX Vars
char* server = "molpe.eafit.edu.co";  //TWX server (do not include http://)
...
void POST(int sensorCount, char* sensorNames[], float values[] ) {
  ...
  for (int idx = 0; idx < sensorCount; idx++)
  {
    ...
    all += sensorNames[idx];
    all += "=";
    all += String(values[idx], 2);
  }
  all += ">";
  if (client.connect(server, 8080)) {
    // send the HTTP POST request:
    client.print(F("POST /Thingworx/Things/"));
    client.print(thingName);
    client.print(F("/Services/"));
    client.print(serviceName);
    client.print(F("?appKey="));
    client.print(appKey);
    client.print(F("&method=post&x-thingworx-session=true"));
    client.print(all);
    ...
    Serial.print(F("POST /Thingworx/Things/"));
    Serial.print(thingName);
    Serial.print(F("/Services/"));
    Serial.print(serviceName);
    Serial.print(F("?appKey="));
    Serial.print(appKey);
    Serial.print(F("&method=post&x-thingworx-session=true"));
    Serial.print(all);
...
void loop() {
  ...
  {
    POST(sensorCount, nameArray, sensorValues);  //This is the library function that constructs the REST call and sends it to your ThingWorx service
